...
// Dynamic social share card per fighter page. Inherits the route's
// generateStaticParams. No external fonts (default sans) so it can't fail.
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          {f?.nickname ? <div style={{ color: "#e11d2a", fontSize: 34, fontStyle: "italic" }}>&ldquo;{f.nickname}&rdquo;</div> : null}
          <div style={{ color: "#fff", fontSize: 90, fontWeight: 800, lineHeight: 1.02, marginTop: 8 }}>{name}</div>
          <div style={{ color: "#9ca3af", fontSize: 34, marginTop: 22 }}>{sub}</div>
        </div>

        <div style={{ color: "#6b7280", fontSize: 24 }}>
          Real stats · Vex AI simulator · fightvex.com
        </div>
      </div>
    ),
    { ...size },
