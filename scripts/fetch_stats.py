...
def main():
    ...
    # MERGE, don't overwrite: load any previously-captured stats first. We only
    # REPLACE a fighter we successfully re-fetch this run, so a transient ESPN
    # failure can never DROP a fighter's existing stats (and newly-added fighters
    # are simply added). This makes the CI's full regen non-destructive.
    existing = {}
    try:
        prev = open(OUT, encoding="utf-8").read()
        me = re.search(r"REAL_AGG: Record<string, RealAgg> = (\{.*\});", prev, re.S)
        if me:
            existing = json.loads(me.group(1))
    except FileNotFoundError:
        pass
    ...
    with ThreadPoolExecutor(max_workers=24) as ex:
        for fid, agg in ex.map(agg_fighter, ids):
            if agg: fetched[fid] = agg
    out = {**existing, **fetched}
    print(f"  re-fetched {len(fetched)}/{len(ids)}; kept {len(out) - len(fetched)} prior; {len(out)} total")
    banner = (
        "// ============================================================\n"
